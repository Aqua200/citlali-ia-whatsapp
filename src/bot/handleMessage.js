// src/bot/handleMessage.js -> VERSIÓN FINAL CON PROTECCIÓN ANTI-CRASH
...
// --- (Las funciones de la base de datos y de búsqueda no cambian) ---
...
function findBestMatch(text, knowledgeArrays) { let allKnowledge = []; if (knowledgeArrays.global) allKnowledge.push(...knowledgeArrays.global); if (knowledgeArrays.local) allKnowledge.push(...knowledgeArrays.local); if (knowledgeArrays.observed) allKnowledge.push(...knowledgeArrays.observed); if (allKnowledge.length === 0) return null; const learnedQuestions = allKnowledge.map(k => k.pregunta); const matches = stringSimilarity.findBestMatch(text, learnedQuestions); if (matches.bestMatch.rating > 0.5) { const bestKnowledge = allKnowledge.find(k => k.pregunta === matches.bestMatch.target); return bestKnowledge; } return null; }
...
export async function handleMessage(sock, msg) {
  ...
  // Si el mensaje no tiene un remitente válido (es un "mensaje fantasma"), lo ignoramos.
  if (!senderJid) {
    console.log("Mensaje recibido sin remitente (probablemente de sincronización), ignorando.");
    return;
  }
  ...
  if (command) {
    ...
    try { await command.execute(sock, msg, args); } catch (error) { console.error(`Error ejecutando el comando ${commandName}:`, error); await sock.sendMessage(from, { text: '¡Ups! Algo salió mal al intentar ejecutar ese comando.' }); }
    return;
  }
  ...
  if (lowerText.includes('género') || lowerText.includes('quién eres')) { return await sock.sendMessage(from, { text: 'Soy una inteligencia artificial con identidad femenina.' }); }
